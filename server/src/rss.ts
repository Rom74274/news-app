...
export const NEWS_SOURCES: RSSSource[] = [
  ...
  { name: "Reuters", url: "https://www.reutersagency.com/feed/?taxonomy=best-sectors&post_type=best", icon: "Reu", category: "monde" },
...
async function fetchSource(source: RSSSource): Promise<RawArticle[]> {
  try {
    ...
    const response = await fetch(source.url, {
      headers: { "User-Agent": "ActuExpress/1.0" },
    ...
    console.warn(`[RSS] Erreur ${source.name}:`, (error as Error).message);
    return [];
  }
}
...
export async function fetchAllRSS(): Promise<RawArticle[]> {
  ...
  // Filtrer : uniquement les articles depuis hier (J-1)
  ...
  // Garder max 10 articles par catégorie (france / monde)
  const france = recent.filter((a) => a.category === "france").slice(0, MAX_ARTICLES_PER_CATEGORY);
  const monde = recent.filter((a) => a.category === "monde").slice(0, MAX_ARTICLES_PER_CATEGORY);

  const capped = [...france, ...monde];
  capped.sort(
    (a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime()
  );
